...
class Settings(object):
	...
	def __init__(self):
		self.settingsFile = open(os.path.join(os.path.dirname(__file__),os.pardir,'config','settings.json'))
		self.data = json.load(self.settingsFile)
		
class ItemChecker(object):

	def __init__(self):
		self.nodesFile = open(os.path.join(os.path.dirname(__file__),os.pardir,'config','nodes.json'))
...
class Helper(object):
	...
	def writeExceptionToFile(self, text):
		try:
			timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
			filePath = self.settings.data["logs.folder.path"]+time.strftime('exceptions/exceptions_%Y%m%d.log')
	...
	def writeLogToFile(self, filename, text):
		try:
			timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
			filePath = self.settings.data["logs.folder.path"]+filename+'.log'
	...
	def __init__(self, helper, name):
		...
		for line in out.splitlines():
			if(name in line):
				...
				try:
						self.helper.deleteFile(helper.settings.data["delay.files.path"]+'{0}.json'.format(name))
				except Exception as e:
