...
try:
	...
	if(int(timearg) > 0):
		...
		if (node["sendOption"] == 2):
				...
				helper.runRadioSwitch(helper.settings.data["conrad.code.sendPath"],str(node[codearg]))
		elif (node["sendOption"] == 0):
				helper.runRadioSwitch(helper.settings.data["code.send.path"],str(node[codearg]))

		helper.deleteFile(helper.settings.data["delay.files.path"]+'{0}.json'.format(namearg))
